...
let MODSUB_REFL = prove
  (`!m x. MODSUB2 m x x = 0`,
    ...
    REWRITE_TAC[MODSUB; ADD_SUB2; MOD_REFL]);;

let ADDL_MODSUB = prove
  (`!m a b. MODSUB2 m (a + b) a = b MOD m`,
   ONCE_REWRITE_TAC[MESON[MODSUB_MODL; MOD_ADD_MOD]
     `MODSUB2 m (a + b) a = MODSUB2 m (a MOD m + b MOD m) a`] THEN
   REWRITE_TAC[MODSUB; MOD_ADD_MODULUS; MOD_MOD_REFL;
     ARITH_RULE `((a MOD m + b MOD m) + m) - a MOD m = b MOD m + m`]);;

let ADDR_MODSUB = prove
  (`!m a b. MODSUB2 m (b + a) a = b MOD m`,
   ONCE_REWRITE_TAC[ADD_SYM] THEN MATCH_ACCEPT_TAC ADDL_MODSUB);;
...
let MODSUB_SUC_LT_EQ = prove
  (`!m a b s. MODSUB2 m a (SUC b) < MODSUB2 m a b <=>
              (if m = 0 then b < a else ~(a == b) (mod m))`,
   REPEAT GEN_TAC THEN EQ_TAC THENL
   [COND_CASES_TAC THENL
    [ASM_REWRITE_TAC[MODSUB_ZERO] THEN ARITH_TAC;
     ONCE_REWRITE_TAC[GSYM CONTRAPOS_THM] THEN
     REWRITE_TAC[NOT_CLAUSES; NOT_LT; CONG] THEN STRIP_TAC THEN
     GEN_REWRITE_TAC LAND_CONV [GSYM MODSUB_MODL] THEN
     POP_ASSUM SUBST1_TAC THEN
     REWRITE_TAC[MODSUB_MODL; MODSUB_REFL; LE_0]];
    COND_CASES_TAC THENL
    [ASM_REWRITE_TAC[MODSUB_ZERO] THEN ARITH_TAC;
     POP_ASSUM MP_TAC THEN REWRITE_TAC[IMP_IMP] THEN
     MATCH_ACCEPT_TAC MODSUB_SUC_LT]]);;
...
let MODRNG_MODSUB = prove
  (`!m a b. ~(m = 0) ==>
   ...
   (* ==> *)
   [STRIP_TAC THEN FIRST_X_ASSUM SUBST1_TAC THEN
    ASM_REWRITE_TAC[MODSUB_MODL; ADDL_MODSUB; MOD_LT_EQ] THEN
    POP_ASSUM MP_TAC THEN ONCE_REWRITE_TAC[GSYM CONTRAPOS_THM] THEN
